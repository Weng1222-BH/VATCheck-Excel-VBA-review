param(
    ...
    [switch]$VerificationSealTestOnly,
    [switch]$VerifiedStateCommitTestOnly
)
...
try {
    ...
    if ($VerifiedStateCommitTestOnly) {
        ...
        $form.Name = 'frmVATCheck'
        $form.CodeModule.AddFromString([IO.File]::ReadAllText((Join-Path $root 'src/frmVATCheck.vba'), [Text.Encoding]::UTF8))
        $result = [string]$excel.Run('VATStage2VerifiedStateCommit_SelfTest')
        [IO.File]::WriteAllText((Join-Path $reportDir 'stage2-verified-state-commit-test-results.txt'), $result, [Text.UTF8Encoding]::new($true))
        Write-Output $result
        if (-not $result.StartsWith('PASS:')) { throw 'C7.6提交协调自测失败，请读取 tests/stage2-verified-state-commit-test-results.txt。' }
        return
    }
    if ($VerificationSealTestOnly) {
        # C7.5只在独立临时目录验证seal，不访问真实baseline或生成release。
        foreach ($moduleName in @('modVATStage2Parser', 'modVATStage2Matcher', 'modVATStage2AIntegrity', 'modVATStage2BRowMatch', 'modVATStage2BConflict', 'modVATStage2ExcelSnapshot', 'modVATStage2CompletedScope', 'modVATStage2FullFallback', 'modVATStage2EffectiveRelations', 'modVATStage2Amount', 'modVATStage2GroupAmount', 'modVATStage2EffectiveAmount', 'modVATStage2ShortSuffixPolicy', 'modVATStage2AuditFindings', 'modVATCheck', 'modVATStage2AggregateGate', 'modVATStage2FinalDecision', 'modVATStage2Fingerprint', 'modVATStage2Digest', 'modVATStage2BaselineStore', 'modVATStage2BoundAuditRun', 'modVATStage2IncrementalDelta', 'modVATStage2VerificationSeal', 'modVATStage2SealTests')) {
            $core = $project.VBComponents.Add(1)
            $core.Name = $moduleName
            $moduleDir = if ($moduleName -eq 'modVATStage2SealTests') { 'tests' } else { 'src' }
            $moduleText = [IO.File]::ReadAllText((Join-Path $root "$moduleDir/$moduleName.bas"), [Text.Encoding]::UTF8)
            $core.CodeModule.AddFromString(($moduleText -replace '(?m)^Attribute VB_Name = .*\r?\n', ''))
        }
        $form = $project.VBComponents.Add(3)
        $form.Name = 'frmVATCheck'
        $form.CodeModule.AddFromString([IO.File]::ReadAllText((Join-Path $root 'src/frmVATCheck.vba'), [Text.Encoding]::UTF8))
        $result = [string]$excel.Run('VATStage2VerificationSeal_SelfTest')
        [IO.File]::WriteAllText((Join-Path $reportDir 'stage2-verification-seal-test-results.txt'), $result, [Text.UTF8Encoding]::new($true))
        Write-Output $result
        if (-not $result.StartsWith('PASS:')) { throw 'C7.5 Seal自测失败，请读取 tests/stage2-verification-seal-test-results.txt。' }
        return
    }
    ...
    if ($BaselineStoreTestOnly) {
        ...
        foreach ($moduleName in @('modVATStage2ExcelSnapshot', 'modVATStage2Fingerprint', 'modVATStage2Digest', 'modVATStage2BaselineStore', 'modVATStage2BaselineTests')) {
            $core = $project.VBComponents.Add(1)
            $core.Name = $moduleName
            $moduleDir = if ($moduleName -eq 'modVATStage2BaselineTests') { 'tests' } else { 'src' }
            $moduleText = [IO.File]::ReadAllText((Join-Path $root "$moduleDir/$moduleName.bas"), [Text.Encoding]::UTF8)
            $core.CodeModule.AddFromString(($moduleText -replace '(?m)^Attribute VB_Name = .*\r?\n', ''))
        }
        $result = [string]$excel.Run('VATStage2BaselineStore_SelfTest')
        [IO.File]::WriteAllText((Join-Path $reportDir 'stage2-baseline-store-test-results.txt'), $result, [Text.UTF8Encoding]::new($true))
        Write-Output $result
        if (-not $result.StartsWith('PASS:')) { throw 'C7.2 baseline自测失败，请读取 tests/stage2-baseline-store-test-results.txt。' }
        return
    }
